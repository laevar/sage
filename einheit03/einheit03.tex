...
\begin{itemize}
\item Mengen in Sage haben den Typ \isage{sets.set}.
\item Es ist eine {\color{blue} ungeordnete} Menge von beliebigen
  Objekten. 
\item Mengen werden als Listen in der Funktion \isage{Set([])} angegeben.
\item Leere Mengen werden durch \isage{leere_menge = Set([])} definiert. 
\item Einträge werden durch Kommata getrennt.
\item (Nummerierter) Zugriff auf Elemente der Menge $M$ kann durch \isage{M[n]} erfolgen ($n\ge0$)
\item Mit \isage{M[i:j]} können auch Intervalle ausgegeben werden.
\end{itemize}
\end{frame}
...
M1 = filter(f,M)
\end{sagein}
wird aus der Menge \isage{M} eine Menge \isage{M1} extrahiert, die aus den
Elementen $x\in M$ besteht, für die \isage{f(x)} eine wahre Aussage
ergibt. \\
\isage{f} ist eine Abbildung auf die Boolschen Werte True/False. 
\end{frame}
...
Wie kann ich prüfen, ob eine Menge \isage{A1} eine Teilmenge einer Menge \isage{A}
ist?
\begin{sagein}
A = Set(range(1,11))
A1 = Set(range(1,3))
...
Ganze Zahlen in Sage haben den Datentyp \isage{Integer}. Man kann
sie addieren, subtrahieren und multiplizieren. Das Ergebnis ist
wieder vom Typ \isage{Integer}. 
\begin{sagein}
...
Rationale Zahlen in Sage haben den Datentyp \isage{rational}. Man kann
...
\item Der Datentyp in Sage ist \isage{RealNumber}
 \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Gleitkommazahlen III}
...
\begin{sagein}
Zp(2,print_mode='digits')(73)
\end{sagein}
\begin{sage}
...1001001
\end{sage}
\begin{sagein}
Zp(8,print_mode='digits',check=False)(73) 
\end{sagein}
\begin{sage}
...111
\end{sage}
...
\begin{itemize}
\item Sei $rd(x)$ die 'gerundete' Gleitkomma-Zahl zu $x \in \mathbb{R}$.  Es
...
% \begin{sagein}
%  Zp(2,print_mode='digits')(245)/Zp(2,print_mode='digits')(100)
% \end{sagein}
% \begin{sage}
% ...1100110011001111.01 
% \end{sage}
...
\begin{frame}[fragile]{Genauigkeit}
\begin{itemize}
...
\begin{itemize}
 \item Approximation durch \isage{float}. Berechnen einer numerischen
...
\isage{parent} & Vaterobjekt; Typ der Zahl\\
\hline
\end{tabular}
\end{center}
\end{frame}
...
\item Betrag $|z|=|(x,y)|:=\sqrt{x^2+y^2}$ (Sage: \isage{abs})
...
\item Mittels \isage{real()} und \isage{imag()} erhält man Real- und
                  Imaginärteil.
\begin{sagein}

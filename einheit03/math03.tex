...
\end{itemize}
\item \textsl{Betrag}: $|z|=|(x,y)|:=\sqrt{x^2+y^2}$ (Sage: \isage{abs})
\end{itemize}


\paragraph{Eigenschaften von $\mathbb{C}$:}
\begin{itemize}
\item \textsl{Fundamentalsatz der Algebra}: Jedes nicht konstante Polynom (mit komplexen Koeffizienten) hat mindestens eine Nullstelle in $\mathbb{C}$. 

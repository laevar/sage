...
$M3=\lbrace1,2,a,b,f,g\rbrace$. 
  \begin{enumerate}
  \item
    Ersetzen Sie das Element $3 \in$ M1 durch die Menge $M2$.
  \item  
    Ersetzen Sie das Element $7 \in$ M2 durch das Element $a$.
  \item
    F\"ugen Sie das Element \{2,3\} der Menge $M2$ hinzu.
  \item
    Bestimmen Sie den Schnitt und die Vereinigung von $M1$, $M2$ und $M3$.
  \item
    Bestimmen Sie die Teilmenge der Potenzmenge von $M1$, die sowohl $2$ als auch $c$
    enth\"alt. 
  \end{enumerate}
\end{aufg}
...
\begin{aufg}
 ...
 \item
    ...
    die $8$-adische Darstellung von $c$.
  \end{enumerate}
\end{aufg}
...
\begin{aufg}
Berechnen Sie mit Hilfe des Befehls \verb+_plus()+ die Summe
\[ \sum_{i=1}^{100} \sum_{j=1}^i \frac{1}{j+i} . \]
\end{aufg}

\begin{aufg}
Nach \verb+x:=10^50/3.0+ sind nur die ersten DIGITS Dezimalstellen in
$x$ garantiert richtig. Was passiert beim Abschneiden der
Nachkommastellen mit \verb+trunc+? Was passiert, wenn man die Anzahl
...
\begin{sage}
>> reset(): a1:=1:
>> (a.(i+1):=a.i+1/i) dollar i=1..200
>> a150
\end{sage}
Erklären Sie den Fehler. Wie können Sie die Voreinstellungen ändern, damit der
Fehler nicht mehr auftritt? Wie muss die Eingabe modifiziert  werden, so dass
es auch ohne Änderungen der Voreinstellungen funktioniert? 
\end{aufg}
\end{document}

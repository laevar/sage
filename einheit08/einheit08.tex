...
diff(<Ausdruck>,x,<anzabl>) 
\end{sagein}
Ableitung bzgl. der Unbekannten $x1$, dann der entstehende Ausdruck bzgl. $x2$, etc. 
In der 2ten Form gibt \isage{<anzabl>} die Anzahl der Ableitung bzgl. \isage{x} an.\\
\textbf{Beispiele:} 
\begin{sagein}
...
\item \alert{Quotientenregel}: $(\frac{f}{g})'(x_0) = \frac{f\,'(x_0) g(x_0) - f(x_0)
...
Das Taylorpolynom $(n-1)$-ten Grades zu einem Ausdruck $f$
...
\begin{frame}[fragile]{taylor - Beispiele}
\begin{sagein}
taylor(1/(1-x),x,0,5)
\end{sagein}
\begin{sage}

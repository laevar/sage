...
\begin{frame}[fragile]{Taylorformel in MuPAD}
MuPAD ist in der Lage ein Taylorpolynom zu einer gegebenen Funktion zu
berechnen. Das Taylorpolynom $(n-1)$-ten Grades zu einem Ausdruck $f$
(mit Unbekannten $x$)
am Entwicklungspunkt $x0$ kann durch
\begin{sage}
>> taylor(f,x=x0,n)
\end{sage}
berechnet werden. Rückgabewert ist ein Objekt vom Typ \verb+Series::Puiseux+.
\end{frame}

\begin{frame}[fragile]{Beispiele}
\begin{sage}
>> taylor1:=taylor(1/(1-x),x=0,5)
\end{sage}
\begin{sage}
           2    3    4      5
  1 + x + x  + x  + x  + O(x )
...
\begin{frame}[fragile]{Visualiserung mit MuPAD I}
\begin{sage}
>> p:=student::plotRiemann(cos(x), x = -PI..PI): 
>> plot(p)
\end{sage}
\begin{center}
%\includegraphics[width=8cm]{figures/riemann.png}
\end{center}
\end{frame}
...
\begin{frame}[fragile]{Integrieren in MuPAD}
\begin{itemize}
 \item Bestimmte Integrale der Form $\int_a^b f(x) dx$ können in MuPAD
berechnet werden durch: {\color{blue} \verb+int(f(x),x=a..b)+}
Dabei ist \verb+f(x)+ ein unbestimmter Ausdruck.
\item Unbestimmte Integrale können durch {\color{blue} \verb+int(f(x),x)+} bestimmt
werden.
\item Numerische Approximationen können durch die Funktion
{\color{blue}\verb+numeric::int+} berechnet werden. 
\end{itemize}
\end{frame}
...
\begin{sage}
         /    a + 1                               \
         | b x                                    |
piecewise| -------- if a <> -1, b ln(x) if a = -1 |
         \   a + 1                                /
\end{sage}
\end{small}
\end{frame}

...
\begin{sage}
  ind
\end{sage}
Der Grenzwert existiert nicht. Sage gibt in diesem Fall \verb+ind+ (indefinite aber beschränkt) zurück. 
...
\begin{itemize}
\item Die Funktionen \verb+plot+ und \verb+plot3d+ dienen
             zur Darstellung von Graphen von Funktionen mit einem
             bzw. zwei Argumenten. 
\item Grafiken werden im Notebook integriert.
\item Die meisten 3D-Grafiken können interaktiv bearbeitet werden.
\end{itemize}
\end{frame} 
...
Erst beim \verb+show()+ oder falls sie in der letzten Zeile ohne Zuweisung stehen, werden die Objekte zu einer gemeinsamen {\color{red} grafischen
...
\verb+aspect_ratio+ & Verhältnis der Achsen (Breite/Höhe). $1$ für 1:1 Verhältnis. \newline
              {\color{blue} \verb+aspect_ratio = 2+}\\
\verb+figsize+    & Grösse des Bildes \newline
                  {\color{blue} \verb+figsize = [width, height]+}\\          
\verb+axes_labels+ &  Tuple oder Liste der Achsenbeschriftungen \newline
{\color{blue} \verb+axes_labels = ('$x$','$y$')+}\\
\verb+gridlines+ & Gitterlinien\newline
              {\color{blue} \verb+gridlines = True+}
\end{tabular}
\end{frame}

\begin{frame}[fragile]{(ein paar) Optionen für grafische Objekte}
\begin{tabular}{lp{8cm}}
\verb+linestyle+  & Darstellung von Linien\newline
           (\verb+'-'+ (solid), \verb+'-.'+ (dashed), \verb+':'+) (dotted)\newline
                {\color{blue} \verb+linestyle = '.'+}\\
\verb+thickness+  & Linienstärke in mm\newline
              {\color{blue} \verb+thickness = 4+}\\
\verb+color+      & Zuweisung einer Farbe\newline
              {\color{blue} \verb+color='red'+}\\
\verb+plot_points+        & Anzahl Stützstellen\newline
              {\color{blue} \verb+plot_points  = [nx,ny]+} (2 Parameter)\\
\verb+alpha+  & Transparent\newline
     {\color{blue} \verb+alpha = 0.8+}\\
\end{tabular}
\end{frame}
...
Der Befehl ist  
\begin{sage}
>> Objekt = parametric_plot([x,y,z],(t,a,b))
\end{sage}
...
\begin{frame}[fragile]{Flächen}
Flächen werden in Sage erzeugt durch
\begin{sage}
...
\end{sage}
\verb+fill=True+ würde die Flächen zwischen den Linien ausfüllen.
\end{frame}

\begin{frame}[fragile]{Punkte zeichnen}
Mittels {\color{blue} \verb+point+} 
...
mit dem Befehl \verb+animate()+ lassen sich einfach Animationen erstellen. 
Beispiele:
\begin{itemize}

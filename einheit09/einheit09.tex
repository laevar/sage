...
\subtitle{Strings, interaktive Grafiken, Komplexe Beispiele, Sage-Code, geogebra ?}
\input{../slide_header}
\maketitle
...
\begin{itemize}
\item Mit dem Indexoperator {\color{blue}$[\ ]$} können einzelne Zeichen einer Zeichenkette
extrahiert werden.
\begin{sagein}
...
\begin{frame}[fragile]{interact - Kontur und 3D-Plot von einer Abstandsfunktion}
\begin{center}
...
\begin{frame}[fragile]{Betrag (noch sehr unschoen)}
\begin{sagein}
def betrag(a):
    if type(a) == Integer or type(a) == Rational or is_RealNumber(a):
        if a>0:
            y = a
        else:
            y = -a
...
\begin{frame}{Gültigkeit von Variablen}
\begin{itemize}
\item Mit der Gültigkeit von Variablen ist die Bestandsdauer von
   Variablen bzw. der Werten dieser Variablen gemeint.
\item \alert{globale Variablen}: Im aktuellen Worksheet sind alle Variablen/Funktionen global, 
...
ändern.
\item \alert{lokale Variablen}: Diese sind nur innerhalb einer Prozedur/Funktion 
...
\begin{itemize}
\item \alert{Hilfetext}: Ein Programm sollte im Hilfetext eine gute Beschreibung der Funktionsweise der Funktion enthalten. Dieser sollte insbesondere Ein- und Ausgabe-Variablen genau beschreiben.
\item \alert{Kommentieren}: Im Programm sollte durch Kommentare dokumentiert werden, was einzelne, wesentliche Abschnitte/Zeilen im Programm tun.
\item \alert{Passende Benennung von Variablen/Funktionen}: Variablen und Funktionen sollten durchdacht benannt werden, d.h. man sollte an ihnen im Optimum direkt erkennen können, welchem Zweck sie im gegebenen Kontext dienen. 
\end{itemize}
\end{frame}

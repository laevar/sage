...
\begin{itemize}
\item Mittels {\color{blue} \isage{filter(Liste, Funktion)}} können Objekte mit bestimmten Eigenschaften aus einer
...
\item Der Befehl \isage{filter} funktioniert auch für Mengen, Dictionaries oder Ausdrücken.
\end{itemize}
\end{frame}
...
\end{sage}
% \begin{sagein}
% def f(arg): return x in [arg]
% filter(f,(11*x*y*(1-x)^2*5).operands())
% \end{sagein}
% \begin{sage}
%            2
%   x (x - 1)
